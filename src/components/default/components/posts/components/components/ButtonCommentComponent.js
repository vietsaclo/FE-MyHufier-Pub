import { Button, Comment, Dropdown, List, Menu, message, Modal, Pagination, Tooltip } from 'antd';
...
import TextBoxCommentComponent from './TextBoxCommentComponent';

// import loadable from '@loadable/component';
// const TitleForModel = loadable(() => import('../../../../../home/header/components/TitleForModel'));
// const TextBoxCommentComponent = loadable(() => import('./TextBoxCommentComponent'));

class ButtonCommentComponentV2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleOk = async () => {
    const CoreUI = await FACTORY.GET_CORE_UI();
    this.refCommentContent.scrollTo({ top: 0, behavior: 'smooth' });
    // user loged ?
  ...
  getTextBoxComment() {
    if (this.state.isEdit)
      return (
        <>
          <div style={{ width: '90%' }} className='float-start'>
            <TextBoxCommentComponent
              value={this.state.commentContent} onChange={(value) => this.tbChange(value)} />
          </div>
          <div style={{ width: '10%' }} className='float-end'>
    ...
    return (
      <>
        ...
        <Modal
          ...
          title={<TitleForModel text='Bình Luận' />} visible={this.state.isModalVisible} onOk={() => this.handleOk()} onCancel={() => this.handleCancel()}>
          <div className='comment-box'>
            <div ref={(ref) => this.refCommentContent = ref} className='comment-content' style={{ overflowY: 'auto', height: `${this.dimension.height - 280}px` }}>
              <>
                <List
...
export default ButtonCommentComponentV2;